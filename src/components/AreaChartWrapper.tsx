...
function processWeeklySummary(data: WeeklySummary[]) {
  return data.map(week => ({
    date: new Date(week.week_start).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
...
export default async function AreaChartWrapper() {
  ...
  const totalVisitors = weeklySummary.reduce((sum, week) => sum + week.visitor_count, 0);
  const overallAvgLoadTime = (weeklySummary.reduce((sum, week) => sum + week.average_load_time, 0) / weeklySummary.length).toFixed(2);
